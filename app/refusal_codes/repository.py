...
def resolve_refusal_code(con, lookup: RefusalCodeLookup) -> RefusalCodeResolution:
    ...
    rows = con.execute(
        """SELECT provider_id, issuer_bank, card_brand, response_code, outcome, reason, source,
                  mapping_version,
    ...
    if not rows:
        return RefusalCodeResolution(ResolutionStatus.NOT_FOUND, value.provider_id, value.issuer_bank,
            value.card_brand, value.response_code, "UNKNOWN", None, None, None)
    best = [row for row in rows if row[8] == rows[0][8]]
    if len({(row[4], row[5], row[6], row[7]) for row in best}) != 1:
    ...
    return RefusalCodeResolution(ResolutionStatus.MATCH_FOUND, value.provider_id, value.issuer_bank,
        value.card_brand if row[2] == "*" else row[2], row[3], row[4], row[5], row[6], row[7])
