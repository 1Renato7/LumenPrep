...
const providerResponseFields: TransactionField[] = ["provider_id", "provider_response_code", "provider_connection_id"];
...
export function TransactionForm({ api: suppliedApi }: TransactionFormProps) {
  ...
  const changeProviderResponse = (rowId: string, field: "provider_response_code" | "provider_connection_id", value: string) => {
    const matched = catalog?.provider_response_options.find((option) => option[field === "provider_response_code" ? "code" : "reason"] === value);
    setForm((current) => current.rows.some((row) => row.id === rowId) ? {
      ...current,
      rows: current.rows.map((row) => row.id === rowId ? {
        ...row,
        values: {
          ...row.values,
          provider_id: matched ? "adyen" : row.values.provider_id,
          provider_response_code: matched ? matched.code : "",
          provider_connection_id: matched ? matched.reason : "",
        },
      } : row),
    } : current);
    setIssues((current) => current.filter((issue) => !(form.rows[issue.rowIndex]?.id === rowId && issue.field !== undefined && providerResponseFields.includes(issue.field))));
    resetIdempotency();
  };
    ...
    <main className={styles.shell}>
      <div className={styles.content}>
        ...
        <form className={styles.batchForm} onSubmit={(event) => void submitBatch(event)} noValidate>
          ...
          <div className={styles.rows}>{form.rows.map((row, rowIndex) => <TransactionRow key={row.id} row={row} rowIndex={rowIndex} options={options} issues={issues.filter((issue) => issue.rowIndex === rowIndex)} canRemove={form.rows.length > 1} canDuplicate={form.rows.length < maximumRows} onChange={changeField} onResponseChange={changeProviderResponse} onDuplicate={() => duplicateRow(row.id)} onRemove={() => removeRow(row.id)} />)}</div>
...
function FieldInput({ field, label, row, type = "text", min, required, error, onChange }: { field: TransactionField; label: string; row: TransactionFormRow; type?: "text" | "number"; min?: string; required?: boolean; error?: string; onChange(rowId: string, field: TransactionField, value: string): void }) { const id = `${row.id}-${field}`; return <div className={styles.field}><label htmlFor={id}>{label}</label><input className={styles.input} id={id} type={type} min={min} required={required} value={row.values[field]} aria-invalid={error ? true : undefined} aria-describedby={error ? `${id}-error` : undefined} onChange={(event) => onChange(row.id, field, event.target.value)} />{error ? <span className={styles.fieldError} id={`${id}-error`} role="alert">{error}</span> : null}</div>; }
